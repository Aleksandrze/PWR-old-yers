...
.data
myArray: .space  12# 40 bajtow to 10 liczb typu int ktory mozem tutaj zmiescic
newLine:.asciiz","
...
.text
	...
	li $v0, 4 	# odczyt 4 w rejester wartosci funkcli
...
inputArray:
	...
	li $t1, 12	#odczyt 40 w rejester $t1 ( rejestr wartosci funkcji)

forArray:
	beq $t0, $t1, bub	 #wykonuj do czasu kiedy t0 i t1 nie sa rowne potem idziemy do bub 
	li $v0,5			# odczyt 5 w rejester wartosci funkcli
	syscall
...
bub:
	jal bubbleSort		#zapamietaj nastepna isntrukcje w rejestre $ra i skocz do label
	...
	forSort: beq $t1, $t2, exit2
		...
		lw $t9, myArray($t1)  	#druga liczna do porownania
 		...
 		bgt $t9,$t8, exit3 	#idz do lab if(scr1>scr2)
			move $t5, $ra 	
			jal swap 	#zapamietaj nastepna isntrukcje w rejestre $ra i skocz do label
			move $ra, $t5
		exit3:
			addi $t1, $t1, 4 
		j forSort 		# j skok(przechodzim) do forSort		
	exit2:	
		addi $t0, $t0, 4	
		j forIf 			# j skok(przechodzim) do forIf
	exit1:	 
